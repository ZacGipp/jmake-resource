 ...
 *
 ====================================================== --*/
//修改后配合框架的CSS,根据需求修改滑动窗口大小和滑动条样式(滑动元素必须有固定宽高)
/*
 * .content {
 width:690px;
 height:594px;
...
ntv.effect.scroll.option = {line: "", bar: "", bar_focus: "", content: "", step: "0",detailHeight:0};
ntv.effect.scroll.init = function (line, bar, bar_focus, content, step, detailHeight) {
    if (line && bar && bar_focus && content) {
        this.option.line = $(line);
        this.option.bar = $(bar);
        this.option.bar_focus = $(bar_focus);
        this.option.content = $(content);
        this.option.step = step ? step : 80;//步长
        this.option.detailHeight = detailHeight;//总高度
...
ntv.effect.scroll.enabled = function () {
    ...
    ntv.navigation.move_right = function () {
        ntv.effect.scroll.disabled(2)
    };
    ntv.navigation.move_left = function () {
        ntv.effect.scroll.disabled(4)
    };
};
...
ntv.effect.scroll.disabled = function (num) {
    ...
    if (next_coord && ntv.navigation.getElmByCoord(next_coord)) {
        // 还原系统焦点移动的事件
        ntv.navigation.move_up = function () {
            this.move_control(1)
        };
        ntv.navigation.move_down = function () {
            this.move_control(3);
        };
        ntv.navigation.move_left = function () {
            this.move_control(4)
...
ntv.effect.scroll.show = function () {
    ...
    this.option.line.style.visibility = "visible";
...
ntv.effect.scroll.hide = function () {
    ...
    this.option.line.style.visibility = "hidden";
...
ntv.effect.scroll.content.init = function (content) {
    ...
    this.attr.height =  ntv.effect.scroll.option.detailHeight/*this.attr.content.scrollHeight*/; // 内容元素的实际高度
    this.attr.view_height = this.attr.content.clientHeight; // 内容元素的可视区域高度
    this.attr.part = Math.ceil((this.attr.height - this.attr.view_height) / ntv.effect.scroll.option.step/*this.attr.view_height*/);
...
ntv.effect.scroll.content.scroll_out = function (type) {
}; // 使用时重写
ntv.effect.scroll.content.scroll_top = function () {
    if (this.attr.curr_part > 1) {
        this.attr.content.scrollTop -= ntv.effect.scroll.option.step/*this.attr.view_height*/;
        this.attr.curr_part--;
    } else
        this.scroll_out(1);
...
ntv.effect.scroll.content.moveback = function () {
    this.attr.content.scrollTop = 0;
    this.attr.curr_part = 1;
    ntv.effect.scroll.slider.attr.bar.style.top = "0px";
    this.attr.part = 0;
    this.attr.line_height = 0;
    this.attr.step = 0;
    ntv.effect.scroll.slider.attr.curr_part = 0;
}
